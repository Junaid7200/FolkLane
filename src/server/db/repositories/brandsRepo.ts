...
export function findAllBrands(db: BetterSqlite3.Database): BrandRow[] {
  const rows = db
    .prepare(
      'SELECT id, name, category, description, created_at, updated_at FROM brands ORDER BY name ASC',
    )
    .all() as BrandRecord[]
...
export function findBrandById(
  ...
  const row = db
    .prepare(
      'SELECT id, name, category, description, created_at, updated_at FROM brands WHERE id = ?',
    )
...
export function upsertBrands(db: BetterSqlite3.Database, brands: BrandRow[]) {
  const stmt = db.prepare(`
    ...
    VALUES (@id, @name, @category, @description, @createdAt, @updatedAt)
