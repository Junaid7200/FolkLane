...
const BRAND_ID = 'royal-garments'
const BASE_URL = 'https://garnetclothing.com'
const COLLECTION_SLUG = 'new-arrivals'
const COLLECTION_PAGE_SIZE = 250
const ROYAL_TARGET_MAX_PRODUCTS = 40
...
async function fetchCollectionPageProducts(
  ...
  try {
    ...
    throw new Error(`Unable to fetch Royal Garments collection page ${page}`)
  }
}
...
async function collectProducts(
  ...
  const hardMax = Math.max(
    1,
    Math.min(opts.maxProducts, ROYAL_TARGET_MAX_PRODUCTS),
  )
  ...
  for (let page = 1; page <= opts.maxPages; page += 1) {
    ...
    for (const row of rows) {
      ...
      if (products.length >= hardMax) return products
    }
...
export async function scrapeRoyalGarments(
  opts: ScrapeOptions,
